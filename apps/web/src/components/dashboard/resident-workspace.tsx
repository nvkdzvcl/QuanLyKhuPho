...
function getInitials(fullName: string) {
  ...
  return words.slice(-2).map((word) => word[0]?.toUpperCase()).join('') || 'CD';
}
      ...
      <aside className="hidden min-h-screen w-64 shrink-0 flex-col bg-gradient-to-b from-[#0b3f9c] via-[#0752bd] to-[#073a89] text-white shadow-xl lg:flex">
        ...
        <div className="p-4">
          <button
            type="button"
            onClick={() => logout()}
            aria-label={`Đăng xuất tài khoản ${user.fullName}`}
            className="flex w-full items-center gap-3 rounded-xl border border-white/20 bg-white/5 p-3 text-left transition hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
          >
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-sm font-bold text-blue-700">{getInitials(user.fullName)}</span>
            <span className="min-w-0 flex-1">
              <span className="block truncate text-sm font-semibold">{user.fullName}</span>
              <span className="block truncate text-xs text-blue-100">Cư dân · {user.neighborhood?.name || 'Khu phố'}</span>
            </span>
            <svg className="h-4 w-4 text-blue-100" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m9 18 6-6-6-6" /></svg>
          </button>
        </div>
      </aside>
          ...
          <div className="flex min-h-[68px] items-center justify-between gap-3 px-4 sm:px-6 lg:min-h-[76px] lg:px-8">
            ...
            <div className="flex shrink-0 items-center gap-2 sm:gap-3">
              ...
              <div className="hidden items-center gap-2 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 sm:flex">
                ...
                <span>Cư dân</span>
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m6 9 6 6 6-6" /></svg>
              </div>
            </div>
          </div>
        </header>
